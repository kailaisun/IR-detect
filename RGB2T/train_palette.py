#!/usr/bin/env python3
"""Train a Palette-style conditional DDPM for RGB -> thermal/pseudo-color."""
...
class PaletteUNet(nn.Module):
    def __init__(self, cond_channels: int = 3, out_channels: int = 1, base: int = 128, emb_dim: int = 128) -> None:
        super().__init__()
...
def main() -> None:
    ...
    loader = DataLoader(
        RGB2TDataset(args.data / "train.csv", target=args.target, training=True),
    ...
    while step < args.steps:
        for rgb, target in loader:
            ...
            pred = model(torch.cat([noisy, rgb], 1), t)
            ...
            if step % args.checkpointing_steps == 0:
                torch.save({"model": model.state_dict(), "step": step}, out / f"checkpoint-{step}.pt")

    torch.save({"model": model.state_dict(), "step": args.steps}, out / "final.pt")

#!/usr/bin/env python3
"""Evaluate a trained SDXL ControlNet (IR -> RGB) on the test split."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--controlnet", type=Path, default=ROOT / "runs" / "controlnet_sdxl" / "final")
    ...
    parser.add_argument("--output", type=Path, default=ROOT / "runs" / "controlnet_sdxl" / "eval")
...
def main() -> None:
    ...
    strips = [np.concatenate([a, b, c], axis=1) for a, b, c in zip(sample_ir, sample_fake, sample_real, strict=True)]
    Image.fromarray(np.concatenate(strips, axis=0)).save(args.output / "samples.png")
